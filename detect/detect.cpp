...
//  medianBlur(frame_gray, frame_gray, 5);
  cascade.detectMultiScale(frame_gray, detections, 
			   1.1, 5, 0|CV_HAAR_SCALE_IMAGE, Size(30, 30));
...
void runonVideo(const CascadeClassifier cascade) {
  ...
  while(!finish) {
    ...
    imshow("video", frame);
    if(waitKey(30) == int('q')) finish = true;
  }
}
...
int main(int argc, char** argv) {
  if(argc != 2 && argc != 3) {
    cerr << argv[0] << ": "
	 << "got " << argc-1 
	 << " arguments. Expecting one or two: cascade [image-folder]" // Cascade file and image folder.
	 << endl
